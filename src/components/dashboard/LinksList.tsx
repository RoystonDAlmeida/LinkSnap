      ...
      <CardContent>
        <div className="w-full overflow-x-hidden">
          ...
          {loading ? (
            ...
            links.map((link) => (
              <div key={link.id} className={`flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 border rounded-lg hover:bg-gray-50 transition-colors ${link.status === "disabled" ? "opacity-60" : ""}`}>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center space-x-2 mb-2">
                    <p
                      className={`font-semibold text-blue-600 break-all ${link.status === "disabled" ? "line-through" : ""}`}
                ...
                {/* Desktop: inline icon group */}
                <div className={`hidden sm:flex items-center space-x-2 ml-4 ${link.status === "disabled" ? "line-through" : ""}`}>
                  <Tooltip>
                    ...
                    <TooltipTrigger asChild>
                      ...
                      >
                        {link.status === "disabled" ? (
                          <a tabIndex={-1} style={{ pointerEvents: "none" }}>
                ...
                {/* Mobile: bottom-right icon group */}
                <div className="flex w-full justify-end mt-4 sm:hidden">
                  <div className={`flex items-center space-x-4 ${link.status === "disabled" ? "line-through" : ""}`}>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button
                          variant="ghost"
